...
class PrecisionFormat:
    ...
    def mul(self, secondNumber):
        ...
        PrecisionFormat.printNumber(newExponeta, newMantisa[1:self.bitForMant + 1:], isNegative, self.bitForMant,
                                    self.bitForExp)
    ...
    def add(self, secondNumber):
        if IsNumberSpecial(self) or IsNumberSpecial(secondNumber):
            ...
            PrecisionFormat.printNumber(newExponenta, newMantisa[1:self.bitForMant + 1:], isNegative, self.bitForMant,
                                        self.bitForExp)
    ...
    def print(self):
        if self.isINF:
            ...
            PrecisionFormat.printNumber(self.exponenta, bin(abs(self.mantisa))[3::], self.isNegative, self.bitForMant,
                                    self.bitForExp)
        ...
        match operation:
            case "+":
                if self.isZero and secondNumber.isZero:
                    PrecisionFormat.printNumber(0, '0', False, self.bitForMant, self.bitForExp)
    ...
    def printNumber(exponenta, mantisa, isNegative, bitForMant, bitForExp):
        if exponenta == 0 and mantisa == "0" or mantisa == "":
            ans = f'{"-" if isNegative else ""}0x0.'
            mantisa = "0"
        else:
